...
async function main() {
  // Konu ataması sınıflandırma dosyasından (müfredat ağacına karşı doğrulandı).
  const konuOf = new Map<string, string>();
  const dersOf = new Map<string, string>();
  for (const s of JSON.parse(readFileSync(`${KOK}/siniflandirma.json`, 'utf8'))) {
    if (s.konuId) { konuOf.set(`s${s.no}`, s.konuId); dersOf.set(`s${s.no}`, s.ders); }
  }
  const asil = new Map<string, any>();
  for (const f of readdirSync(`${KOK}/parti`).filter((x) => /-kor\.json$/.test(x) && !/^(onarilan2?|hakem)-/.test(x)))
    for (const q of JSON.parse(readFileSync(`${KOK}/parti/${f}`, 'utf8')).sorular) asil.set(q.id, q);
  ...
  for (const o of oneriler) {
    ...
    gecerli.push({ ...o, kok, siklar, konuId: konuOf.get(o.id)!, ders: dersOf.get(o.id) ?? '—' });
  }
  ...
  const dag = yazilacak.reduce<Record<string, number>>((a, r) => ({ ...a, [r.ders]: (a[r.ders] ?? 0) + 1 }), {});
