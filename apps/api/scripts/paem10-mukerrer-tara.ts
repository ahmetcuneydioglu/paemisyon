/**
 * Doc 36 — SALT OKUMA: PAEM 10 sorularını bankaya karşı mükerrere tarar.
 *
 * Yalnız B kitapçığı taranır; A aynı soruların karışığıdır (ayrıştırıcı
 ...
 *
 *   npx tsx scripts/paem10-mukerrer-tara.ts
 */
...
async function main() {
  const A = JSON.parse(readFileSync(`${KOK}/ham/paem10-B-sorular.json`, 'utf8'));
  ...
  console.log(`PAEM 10 girdi : ${A.length}`);
